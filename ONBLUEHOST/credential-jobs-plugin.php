...
function credential_render_job_listings() {
    ...
    if ($job_count > 0) {
        foreach ($results as $row) {
            ...
            // Create job detail URL - preserve all URL parameters
...
function credential_render_job_detail($job_id) {
    ...
    // Back to listings link - preserve all parameters except job_id
    $back_url = remove_query_arg('job_id');
    echo '<a href="' . esc_url($back_url) . '" class="back-link"><i class="fas fa-arrow-left"></i> Back to all credential jobs</a>';
    ...
    // Build apply now URL with job_id and required parameters
    $apply_now_url = "https://skillsforchicago.org/candidate-login/?tfa_3=" . urlencode($job->JN) . "&cred=credential";
