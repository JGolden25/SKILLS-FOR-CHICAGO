...
function skills_content_has_html($content) {
    return preg_match('/<[^>]*>/', $content);
}

// Function to sanitize output - modified to handle HTML content properly
function skills_clean_detail_output($data, $allow_html = false) {
    if ($allow_html) {
...
function skills_render_job_detail($job_id) {
    ...
    $cid = isset($_GET['CID']) ? $_GET['CID'] : ''; // Added CID parameter
    ...
    if (!empty($job->{'Job Description'})) {
        ...
        if (skills_content_has_html($description)) {
            // If it has HTML, just display it with safe HTML allowed
            echo '<div class="job-detail-content" style="font-size: 22pt !important;">' . skills_clean_detail_output($description, true) . '</div>';
        } else {
            // If no HTML, format with bullets as before
            $description = nl2br(skills_clean_detail_output($description));
    ...
    if (!empty($job->{'Skills and Qualifications'})) {
        ...
        if (skills_content_has_html($skills)) {
            // If it has HTML, just display it with safe HTML allowed
            echo '<div class="job-detail-content" style="font-size: 22pt !important;">' . skills_clean_detail_output($skills, true) . '</div>';
        } else {
            // If no HTML, format with bullets as before
            $skills = nl2br(skills_clean_detail_output($skills));
